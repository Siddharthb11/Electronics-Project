...
from .models import Products_Table, Cart_Table, Order_Table, CustomerDetails
...
def home(request):
    ...
    fetched_porducts = Products_Table.objects.filter(is_active=True)
...
def filter_by_category(request, category_value):
    ...
    Products_Table.objects.filter(q1 & q2)
    filtered_products = Products_Table.objects.filter(q1 & q2)
...
def sort_by_brand(request, brand_name):
    ...
    Products_Table.objects.filter(q1&q2)
    sorted_products = Products_Table.objects.filter(q1&q2)
...
def filter_by_price(request):
    ...
    filtered_products = Products_Table.objects.filter(q1 & q2 & q3)
    data['products'] = filtered_products
    return render(request, 'products/store.html', context=data)
...
def sort_by_rating(request, rating_value):
    ...
    rating_product = Products_Table.objects.filter(q1 & q3)
...
def add_to_cart(request,pid):
    if request.user.is_authenticated:
        ...
        product = Products_Table.objects.get(id = pid)
        q1 = Q(uid = uid)
        q2 = Q(pid = pid)
